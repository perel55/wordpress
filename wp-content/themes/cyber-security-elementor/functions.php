...
function cyber_security_elementor_enqueue_google_fonts() {
	...
	wp_enqueue_style(
		'outfit',
		wptt_get_webfont_url( 'https://fonts.googleapis.com/css2?family=Outfit:wght@100..900&display=swap' ),
		array(),
		'1.0'
	);
	...
	wp_enqueue_style(
		'comic-neue',
		wptt_get_webfont_url( 'https://fonts.googleapis.com/css2?family=Comic+Neue:ital,wght@0,300;0,400;0,700;1,300;1,400;1,700&display=swap' ),
		array(),
		'1.0'
	);
	...
	echo '<div class="notice notice-info wpele-activation-notice is-dismissible">';
		echo '<div class="notice-body">';
			...
			echo '<div class="notice-content">';
				...
				echo '<span><a href="'. esc_url( admin_url( 'themes.php?page=cyber_security_elementor_about' ) ) .'" class="button button-primary">'. esc_html__( 'GET STARTED', 'cyber-security-elementor' ) .'</a></span>';
			echo '</div>';
		echo '</div>';
	echo '</div>';
}
